<?php
    ...
    <head>
        ...
        <style>
            ...
            .offcanvas .nav-link, .offcanvas .offcanvas-title {
                color: #fff !important;
            }
            .card {
                border-radius: 16px;
            ...
            .custom-img-size, .preview-image, video {
                border-radius: 10px;
                box-shadow: 0 2px 8px rgba(0,0,0,0.10);
            }
            .modal-content {
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="actualizar2.php" method="post" enctype="multipart/form-data">
                        <div class="mb-2">
      ...
      <!-- Barra de navegación -->
            <nav class="navbar navbar-dark shadow-sm" style="background: #218838;">
                <div class="container-fluid d-flex justify-content-between align-items-center">
                    ...
                    <button class="navbar-toggler ms-auto" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasNavbar" aria-controls="offcanvasNavbar" aria-label="Menú">
            ...
            <div class="offcanvas offcanvas-end custom-offcanvas" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
                <div class="offcanvas-header d-flex align-items-center justify-content-between">
                    <h3 class="offcanvas-title fw-bold" id="offcanvasNavbarLabel" style="color:#fff;">Menú</h3>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
                <div class="offcanvas-body">
                    <ul class="navbar-nav flex-column gap-3">
                        <li class="nav-item">
                            <a class="nav-link active menu-link" aria-current="page" href="usuarios.php">
                                <span class="d-flex align-items-center gap-2 fs-4 fw-bold">
                                    <i class="bi bi-person-plus-fill"></i> Registro Usuario
                                </span>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active menu-link" aria-current="page" href="cerrarsesion.php"> 
                                <span class="d-flex align-items-center gap-2 fs-4 fw-bold">
                                    <i class="bi bi-person-plus-fill"></i> Cerrar Sesión
                                </span>
                            </a>
                        </li>
                    </ul>
        ...
        <div class="container py-3">
            ...
            <?php if (isset($_SESSION['msg']) && isset($_SESSION['color'])) { ?>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    showToast("<?= addslashes($_SESSION['msg']); ?>", "<?= $_SESSION['color'] == 'success' ? 'bg-success' : ($_SESSION['color'] == 'danger' ? 'bg-danger' : 'bg-primary') ?>");
                });
                </script>
            <?php
                        ...
                        <?php
                            ...
                            for ($i = 0; $i < $tam; $i++) {
                                ...
                                if (file_exists($imagePath)) {
                                    $linkId = 'modalLink' . $row['id'];
                                    echo '
                                    <a href="' . $imagePath . '" target="_blank" class="d-block mb-3">
                                        <img src="' . $imagePath . '?n=' . time() . '" class="img-fluid custom-img-size" alt="Image">
                                    </a>';
                                } else {
    ...
    function showToast(message, colorClass = 'bg-success') {
        ...
        var toastHtml = `
        <div id="${toastId}" class="toast align-items-center text-white ${colorClass} border-0 mb-2" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="3000">
            <div class="d-flex">
        ...
        <script>
            ...
            nuevoModal.addEventListener('hide.bs.modal', event => {
                const categoria = nuevoModal.querySelector('.modal-body #categoria');
                if (categoria) categoria.value = "";
                const numeroerrores = nuevoModal.querySelector('.modal-body #numeroerrores');
                if (numeroerrores) numeroerrores.value = "";
                const descripcion = nuevoModal.querySelector('.modal-body #descripcion');
                if (descripcion) descripcion.value = "";
                const solucionoperativa = nuevoModal.querySelector('.modal-body #solucionoperativa');
                if (solucionoperativa) solucionoperativa.value = "";
                const soluciontecnica = nuevoModal.querySelector('.modal-body #soluciontecnica');
                if (soluciontecnica) soluciontecnica.value = "";
                const poster = nuevoModal.querySelector('.modal-body #poster');
                if (poster) poster.value = "";
            })

            editaModal.addEventListener('hide.bs.modal', event => {
                const idInput = editaModal.querySelector('.modal-body #id ');
                if (idInput) idInput.value = "";
                const categoria = editaModal.querySelector('.modal-body #categoria');
                if (categoria) categoria.value = "";
                const numeroerrores = editaModal.querySelector('.modal-body #numeroerrores');
                if (numeroerrores) numeroerrores.value = "";
                const descripcion = editaModal.querySelector('.modal-body #descripcion');
                if (descripcion) descripcion.value = "";
                const solucionoperativa = editaModal.querySelector('.modal-body #solucionoperativa');
                if (solucionoperativa) solucionoperativa.value = "";
                const soluciontecnica = editaModal.querySelector('.modal-body #soluciontecnica');
                if (soluciontecnica) soluciontecnica.value = "";
                const imgPoster = editaModal.querySelector('.modal-body #img_poster');
                if (imgPoster) imgPoster.value = "";
                const poster = editaModal.querySelector('.modal-body #poster');
                if (poster) poster.value = "";
                const imagePreview = editaModal.querySelector('.modal-body #editImagePreview');
                if (imagePreview) imagePreview.innerHTML = '';
                const videoPreview = editaModal.querySelector('.modal-body #editVideoPreview');
            ...
            editaModal.addEventListener('shown.bs.modal', event => {
                ...
                let fecha = editaModal.querySelector('.modal-body #fecha');
                ...
                // Asignar el ID al campo idd antes de la petición fetch
                if (idd) idd.value = id;
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        if (idd && inputCategoria && inputNumeroErrores && inputDescripcion && inputSolucionOperativa && inputSolucionTecnica) {
                            idd.value = data.id;
                            inputCategoria.value = data.id_categoria;
                            inputNumeroErrores.value = data.numeroerrores;
                            inputDescripcion.value = data.descripcion;
                            inputSolucionOperativa.value = data.solucionoperativa;
                            inputSolucionTecnica.value = data.soluciontecnica;
                        }
                        if (fecha) {
                            fecha.value = data.fecha;
                        }
                        // Mostrar imagen actual en tamaño normal
                        if (imagePreview) {
                            let imgHtml = '';
                            if (data.poster) {
                                let imgs = data.poster.split(',');
                                imgs.forEach(function(img) {
                                    let imgPath = img.startsWith('posters/') ? img : 'posters/' + img;
                                    imgHtml += `<img src="${imgPath}?n=${Date.now()}" class="img-fluid mb-2" style="max-width:100%;height:auto;">`;
                                });
                            } else {
                                imgHtml = '<p>No hay imagen disponible</p>';
                            }
                        ...
                        if (videoPreview) {
                            ...
                            if (data.id) {
                                ...
                                videoHtml = `<video style="max-width:200px;max-height:120px;" controls><source src="${videoPath}" type="video/mp4">Tu navegador no soporta el elemento de video.</video>`;
                            } else {
                                videoHtml = '<p>No hay video disponible</p>';
                            }
                            videoPreview.innerHTML = videoHtml;
                        }
            ...
            function previewImages(files, containerId = 'imagePreview') {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if (!file.type.match('image.*')) {
                        ...
                        errorMsg.innerText = 'Error: El archivo seleccionado no es una imagen válida.';
            ...
            function previewVideo(files, containerId = 'editVideoPreview') {
                ...
                if (files.length > 0) {
                    ...
                    if (!file.type.match('video.*')) {
                        var errorMsg = document.createElement('div');
                        errorMsg.className = 'alert alert-danger mt-2';
                        errorMsg.innerText = 'Error: El archivo seleccionado no es un video válido.';
                        previewContainer.appendChild(errorMsg);
                        return;
                    }
                    var reader = new FileReader();
    ...
    <script src="assets/js/bootstrap.bundle.min.js"></script>
                        ...
                        <?php
                            ...
                            for ($i = 0; $i < $tam; $i++) {
                                $imagePath = $imgs[$i];
                                $imagePath = (strpos($imagePath, 'posters/') === 0) ? $imagePath : 'posters/' . $imagePath;
                                if (file_exists($imagePath)) {
                                    $linkId = 'modalLink' . $row['id'];
                                    echo '
                                    <a href="' . $imagePath . '" target="_blank" class="d-block mb-3">
                                        <img src="' . $imagePath . '?n=' . time() . '" class="img-fluid" style="max-width:100%;height:auto;" alt="Image">
                                    </a>';
                                } else {
                                    echo '<p>Imagen no encontrada</p>';
                                }
